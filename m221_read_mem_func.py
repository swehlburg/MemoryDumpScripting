"""
    Original code written by Adeen Ayub
    Stephen Wehlburg added the use of a function instead of just main()
"""
...
def func(plc_ip, start_addr, size, output_file):
    m221_injector = M221_cl_injector(plc_ip)
    mem_block = m221_injector.read_mem(start_addr, size)

    f = output_file(output_file, 'w')
    f.write(mem_block)
    f.close()


def test(plc_ip, start_addr, size, output_file):
    print(output_file)
